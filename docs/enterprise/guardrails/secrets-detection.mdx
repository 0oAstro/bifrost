...
Create the Secrets Detection provider configuration directly with the management API. The provider type is the path segment (`secrets`); the API assigns the configuration ID after creation.
